import esphome.codegen as cg
from esphome.components import sensor
import esphome.config_validation as cv
from esphome.const import (
    DEVICE_CLASS_BATTERY,
    DEVICE_CLASS_CURRENT,
    DEVICE_CLASS_POWER,
    DEVICE_CLASS_TEMPERATURE,
    DEVICE_CLASS_VOLTAGE,
    STATE_CLASS_MEASUREMENT,
    STATE_CLASS_TOTAL_INCREASING,
    UNIT_AMPERE,
    UNIT_CELSIUS,
    UNIT_EMPTY,
    UNIT_PERCENT,
    UNIT_VOLT,
    UNIT_WATT,
)

from . import CONF_HUMSIENK_BMS_BLE_ID, HUMSIENK_BMS_BLE_COMPONENT_SCHEMA

DEPENDENCIES = ["humsienk_bms_ble"]

CODEOWNERS = ["@syssi"]

UNIT_AMPERE_HOURS = "Ah"

MAX_CELLS = 24
MAX_CELL_TEMPS = 4

CONF_TOTAL_VOLTAGE = "total_voltage"
CONF_CURRENT = "current"
CONF_POWER = "power"
CONF_STATE_OF_CHARGE = "state_of_charge"
CONF_STATE_OF_HEALTH = "state_of_health"
CONF_CAPACITY_REMAINING = "capacity_remaining"
CONF_FULL_CHARGE_CAPACITY = "full_charge_capacity"
CONF_CHARGING_CYCLES = "charging_cycles"
CONF_MOSFET_TEMPERATURE = "mosfet_temperature"
CONF_ENVIRONMENT_TEMPERATURE = "environment_temperature"
CONF_MIN_CELL_VOLTAGE = "min_cell_voltage"
CONF_MAX_CELL_VOLTAGE = "max_cell_voltage"
CONF_MIN_VOLTAGE_CELL = "min_voltage_cell"
CONF_MAX_VOLTAGE_CELL = "max_voltage_cell"
CONF_DELTA_CELL_VOLTAGE = "delta_cell_voltage"
CONF_AVERAGE_CELL_VOLTAGE = "average_cell_voltage"
CONF_PROBLEM_BITMASK = "problem_bitmask"


def cell_voltage_schema():
    return sensor.sensor_schema(
        unit_of_measurement=UNIT_VOLT,
        accuracy_decimals=3,
        device_class=DEVICE_CLASS_VOLTAGE,
        state_class=STATE_CLASS_MEASUREMENT,
    )


def temperature_schema():
    return sensor.sensor_schema(
        unit_of_measurement=UNIT_CELSIUS,
        accuracy_decimals=0,
        device_class=DEVICE_CLASS_TEMPERATURE,
        state_class=STATE_CLASS_MEASUREMENT,
    )


SENSORS = {
    CONF_TOTAL_VOLTAGE: sensor.sensor_schema(
        unit_of_measurement=UNIT_VOLT,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_VOLTAGE,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_CURRENT: sensor.sensor_schema(
        unit_of_measurement=UNIT_AMPERE,
        accuracy_decimals=2,
        device_class=DEVICE_CLASS_CURRENT,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_POWER: sensor.sensor_schema(
        unit_of_measurement=UNIT_WATT,
        accuracy_decimals=1,
        device_class=DEVICE_CLASS_POWER,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_STATE_OF_CHARGE: sensor.sensor_schema(
        unit_of_measurement=UNIT_PERCENT,
        accuracy_decimals=0,
        device_class=DEVICE_CLASS_BATTERY,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_STATE_OF_HEALTH: sensor.sensor_schema(
        unit_of_measurement=UNIT_PERCENT,
        accuracy_decimals=0,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_CAPACITY_REMAINING: sensor.sensor_schema(
        unit_of_measurement=UNIT_AMPERE_HOURS,
        accuracy_decimals=3,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_FULL_CHARGE_CAPACITY: sensor.sensor_schema(
        unit_of_measurement=UNIT_AMPERE_HOURS,
        accuracy_decimals=3,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_CHARGING_CYCLES: sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=0,
        state_class=STATE_CLASS_TOTAL_INCREASING,
    ),
    CONF_MOSFET_TEMPERATURE: temperature_schema(),
    CONF_ENVIRONMENT_TEMPERATURE: temperature_schema(),
    CONF_MIN_CELL_VOLTAGE: cell_voltage_schema(),
    CONF_MAX_CELL_VOLTAGE: cell_voltage_schema(),
    CONF_MIN_VOLTAGE_CELL: sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=0,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_MAX_VOLTAGE_CELL: sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=0,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
    CONF_DELTA_CELL_VOLTAGE: cell_voltage_schema(),
    CONF_AVERAGE_CELL_VOLTAGE: cell_voltage_schema(),
    CONF_PROBLEM_BITMASK: sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=0,
        state_class=STATE_CLASS_MEASUREMENT,
    ),
}

CONFIG_SCHEMA = HUMSIENK_BMS_BLE_COMPONENT_SCHEMA.extend(
    {cv.Optional(key): schema for key, schema in SENSORS.items()}
).extend(
    {
        cv.Optional(f"cell_voltage_{i + 1}"): cell_voltage_schema()
        for i in range(MAX_CELLS)
    }
).extend(
    {
        cv.Optional(f"temperature_{i + 1}"): temperature_schema()
        for i in range(MAX_CELL_TEMPS)
    }
)


async def to_code(config):
    hub = await cg.get_variable(config[CONF_HUMSIENK_BMS_BLE_ID])
    for key in SENSORS:
        if key in config:
            sens = await sensor.new_sensor(config[key])
            cg.add(getattr(hub, f"set_{key}_sensor")(sens))

    for i in range(MAX_CELLS):
        key = f"cell_voltage_{i + 1}"
        if key in config:
            sens = await sensor.new_sensor(config[key])
            cg.add(hub.set_cell_voltage_sensor(i, sens))

    for i in range(MAX_CELL_TEMPS):
        key = f"temperature_{i + 1}"
        if key in config:
            sens = await sensor.new_sensor(config[key])
            cg.add(hub.set_temperature_sensor(i, sens))
